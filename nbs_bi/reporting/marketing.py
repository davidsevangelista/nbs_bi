...
class MetaAdsSection:
    ...
    def _build_pdf_bytes(  # pragma: no cover
        ...
        ``st.button`` — which would require two clicks and produce a
        chart-free PDF on the second rerun.
    ...
    def _render_export_button(  # pragma: no cover
        ...
        Uses ``st.session_state`` to store generated bytes so the download
        button persists across Streamlit reruns without regenerating the PDF on
        every page load. PDF is built only when the user explicitly clicks
        "Prepare PDF", which also surfaces kaleido errors via ``st.error``
        instead of silently producing a chart-free document.
